...
class _BillingScreenState extends State<BillingScreen> {
  ...
  Future<void> _scanBarcode() async {
    ...
    try {
      ...
      if (isWeightBased) {
        // Show weight input dialog for weight-based products
        _showWeightInputDialog(product);
      } else {
        // Add 1 piece for count-based products
          ...
          ElevatedButton(
            onPressed: () {
              try {
                ...
                Provider.of<CartProvider>(context, listen: false).addItem(product, quantity: weight);
                Navigator.of(ctx).pop();
              } catch (e) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Invalid weight: ${e.toString()}')),
                );
              }
            },
  ...
  void _showItemEditDialog(BuildContext context, CartItem item, int index) {
    ...
    // Check if product is weight-based
    final isWeightBased = item.isWeightBased;
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration: InputDecoration(
                labelText: isWeightBased ? 'Weight (${item.product.unit})' : 'Quantity',
                border: OutlineInputBorder(),
            ...
            Consumer<CartProvider>(
              builder: (context, cart, child) {
                final qty = double.tryParse(_itemQuantityController.text) ?? item.quantity;
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              try {
                final quantity = double.parse(_itemQuantityController.text);
                    ...
                    builder: (context, cart, child) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isWeightBased = item.isWeightBased;
                          final quantityDisplay = isWeightBased 
